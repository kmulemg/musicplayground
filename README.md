...
- 🎵 **搜索排序**：搜索/歌单结果支持按歌曲、歌手、专辑、时长、大小客户端排序
- 🍪 **夸克网盘**：聚合下载站源（15 个）支持夸克网盘 Cookie，下载音质提升
...
- 修复任务页文件下载链接损坏（相对路径丢失目录结构）
- 修复依赖版本比较 `~=` 逻辑不符合 PEP 440 的问题
- 修复依赖版本缓存永不过期（加入 10 分钟 TTL 判断）
- 修复 Cookie / 源变更时绕过锁直接置空客户端导致的竞态问题
- 修复内存持续增长无上限（新增 `_trim_library` / `_trim_jobs` 自动淘汰旧条目）

### 2026-08-18 · 夸克网盘 Cookie 支持 + 聚合下载站修复

- 新增夸克网盘 Cookie 配置，保存后自动注入到 15 个聚合下载站源
- 修复未配置夸克 Cookie 时 FiveSong / KKWS / LiziYY / Xiageba 源的客户端构造崩溃
- 修复 `request_overrides` 全局注入导致 TwoT58 / ITingWa / GDStudio 等源搜索失败的问题
- 搜索/歌单结果支持按 **歌曲、歌手、专辑、时长、大小** 客户端排序

### 2026-08-17 · 初始版本 v1.0

- 多平台搜索、歌单解析、双下载模式（最高音质 / FLAC→AAC）
- 智能去重、自动整理、文件页分格式下载
- Cookie 管理：手动录入 + 浏览器一键导入
- 在线试听、任务明细、运行日志、重启服务按钮
